...
export function ManagersCheckinReport({ data }: { data: Record<string, any> }) {
  ...
  const kpisForFollowup = Array.isArray(data.kpisMarkedForFollowup) ? data.kpisMarkedForFollowup : [];
      ...
      {kpisForFollowup.length > 0 && (
        <ReportSection title={`KPIs para acompanhamento (${kpisForFollowup.length})`} icon={<BarChart3 className="h-4 w-4" />}>
          <div className="flex flex-wrap gap-2">
            {kpisForFollowup.map((id: string, i: number) => (
              <Badge key={i} variant="outline" className="text-xs">{typeof id === 'string' ? id.slice(0, 8) + '…' : JSON.stringify(id)}</Badge>
            ))}
          </div>
        </ReportSection>
      )}
