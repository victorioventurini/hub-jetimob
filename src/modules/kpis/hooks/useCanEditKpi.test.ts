...
describe('useCanEditKpi', () => {
  ...
  it('returns false everywhere when kpi is null', async () => {
    setup({});
    const { result } = renderHook(() => useCanEditKpi(null), { wrapper: makeWrapper() });
  ...
  it('returns false when profileId is null', async () => {
    ...
    const { result } = renderHook(() =>
      useCanEditKpi({ id: 'k1', bu_id: 'bu-1', scope: 'team', team_id: 't1' }), { wrapper: makeWrapper() }
    );
    await waitFor(() => expect(result.current.isLoading).toBe(false));
    expect(result.current.canEdit).toBe(false);
  });
  ...
  it('grants both canEdit and canUpdateValues for wildcard admin', async () => {
    ...
    const { result } = renderHook(() =>
      useCanEditKpi({ id: 'k1', bu_id: 'bu-1', scope: 'org' }), { wrapper: makeWrapper() }
    );
    await waitFor(() => expect(result.current.isLoading).toBe(false));
  ...
  it('grants canEdit with kpis.settings.manage:bu permission', async () => {
    ...
    const { result } = renderHook(() =>
      useCanEditKpi({ id: 'k1', bu_id: 'bu-1', scope: 'area' }), { wrapper: makeWrapper() }
    );
    await waitFor(() => expect(result.current.isLoading).toBe(false));
    expect(result.current.canEdit).toBe(true);
  });
  ...
  it('denies non-admin from editing area-scoped KPI', async () => {
    setup({ manageableTeamIds: ['t1'] });
    const { result } = renderHook(() =>
      useCanEditKpi({ id: 'k1', bu_id: 'bu-1', scope: 'area', area_id: 'a1' }), { wrapper: makeWrapper() }
    );
    await waitFor(() => expect(result.current.isLoading).toBe(false));
    expect(result.current.canEdit).toBe(false);
  });
  ...
  it('grants canEdit to KPI owner (team scope)', async () => {
    ...
    const { result } = renderHook(() =>
      useCanEditKpi({ id: 'k1', bu_id: 'bu-1', scope: 'team', team_id: 't-other', owner_user_id: 'me-1' }), { wrapper: makeWrapper() }
    );
    await waitFor(() => expect(result.current.isLoading).toBe(false));
    expect(result.current.canEdit).toBe(true);
  });
