/**
 * Espelho de frontend do catálogo canônico de escopos das chaves de API por BU.
 *
 * Fonte de verdade: `supabase/functions/_shared/bu-api-scopes.ts`.
 * Mantido em sincronia por teste (`scopes.test.ts`).
 */
...
export function describeScopes(scopes: string[]): string {
  const levels = levelsFromScopes(scopes);
  const parts = BU_API_MODULES.filter((m) => levels[m.key] !== 'none').map(
    (m) => `${m.label} (${levels[m.key] === 'write' ? 'leitura e escrita' : 'leitura'})`,
  );
  return parts.length ? parts.join(' · ') : 'Nenhum módulo liberado';
}
