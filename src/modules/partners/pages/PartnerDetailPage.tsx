      ...
      <div className="space-y-6 max-w-3xl">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Ative ou desative este parceiro em cada unidade de negócio
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
            {userBus?.map((bu) => {
                const association = associations?.find((a) => a.bu_id === bu.bu_id);
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className="h-8 w-8 rounded-full flex items-center justify-center text-xs font-medium bg-primary text-primary-foreground"
                      >
                        {bu.bu_id.slice(0, 2).toUpperCase()}
                      </div>
                      <div>
                        <p className="font-medium">{bu.bu_id}</p>
                        {bu.bu_id === currentBuId && (
                          <p className="text-xs text-muted-foreground">BU atual</p>
                        )}
                      </div>
                    </div>

                    <div className="flex items-center gap-3">
                      <Label htmlFor={`bu-${bu.bu_id}`} className="text-sm text-muted-foreground">
                        {isActive ? "Ativo" : "Inativo"}
                      </Label>
                      <Switch
                        id={`bu-${bu.bu_id}`}
                        checked={isActive}
                        onCheckedChange={() => handleToggleBu(bu.bu_id, isActive)}
                        disabled={toggleAssociation.isPending}
                      />
                    </div>
                  </div>
                );
