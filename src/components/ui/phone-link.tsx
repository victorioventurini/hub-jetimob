...
export function PhoneLink({ phone, showIcon = true, className }: PhoneLinkProps) {
  ...
  return (
    <a
      ...
      className={`inline-flex items-center gap-1.5 text-foreground hover:text-primary transition-colors ${className || ''}`}
...
/**
 * Simple phone display without WhatsApp link
 */
export function PhoneDisplay({ phone, showIcon = true, className }: PhoneLinkProps) {
  if (!phone) {
    return <span className="text-muted-foreground">-</span>;
  }
