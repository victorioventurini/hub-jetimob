...
-- RPC para listar usuários da BU baseado em memberships (não profiles.bu_id)
-- Isso garante consistência com /hub/users

CREATE OR REPLACE FUNCTION public.get_bu_users_by_membership(
  p_bu_id uuid,
  p_search text DEFAULT NULL,
  p_team_id uuid DEFAULT NULL,
  p_status text DEFAULT 'active', -- 'all', 'active', 'vacation', 'terminated', 'external'
...
COMMENT ON FUNCTION get_bu_users_by_membership IS 'Lista usuários da BU baseado em memberships (não profiles.bu_id). Garante consistência com /hub/users.';
