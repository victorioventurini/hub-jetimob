-- FIX: search_mention_candidates deve excluir usuários com role_in_bu='external' da lista "internal_users"
-- Usuários externos (role_in_bu='external') são contatos vinculados, não colaboradores internos

CREATE OR REPLACE FUNCTION public.search_mention_candidates(
  p_bu_id uuid, 
  p_partner_company_id uuid DEFAULT NULL::uuid, 
...
BEGIN
  ...
  WITH internal_users AS (
    ...
    INNER JOIN bu_user_memberships bum ON bum.profile_id = p.id 
      ...
      -- FIX: Excluir usuários externos (eles são mostrados via partner_contacts)
      AND bum.role_in_bu IN ('admin', 'collaborator')
