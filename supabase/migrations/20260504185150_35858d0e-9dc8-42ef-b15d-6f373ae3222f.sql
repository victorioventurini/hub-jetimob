...
BEGIN
  ...
  v_role := CASE WHEN v_is_external THEN 'external'::public.app_role ELSE 'collaborator'::public.app_role END;
  ...
  ELSE
    INSERT INTO public.profiles (
      user_id, first_name, last_name, display_name, work_email, work_mode,
      city, state, start_date, employment_status, user_type, bu_id, onboarding_completed
    ) VALUES (
      NEW.id, v_first_name,
      COALESCE(NULLIF(v_last_name, ''), 'Parceiro'),
      v_display_name, v_email,
      'hybrid'::public.work_mode,
      'Porto Alegre', 'RS', CURRENT_DATE,
      (CASE WHEN v_is_external THEN 'external' ELSE 'active' END)::public.employment_status,
  ...
  VALUES (NEW.id, v_new_profile_id, v_bu_id, v_role, true)
