-- Fix trigger to use external_company_bu_associations (was partner_company_bu_associations)
...
BEGIN
  -- Apenas tickets externos com external_company_id
  IF NEW.type != 'external' OR NEW.external_company_id IS NULL THEN
    RETURN NEW;
  END IF;

  -- Buscar supervisores da empresa na BU (FIX: was partner_company_bu_associations)
  SELECT 
