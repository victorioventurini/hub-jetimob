...
-- Part 1: Fix handle_new_user to preserve existing profile's bu_id
...
BEGIN
  ...
  -- Check for pre-existing profile (imported before first login)
  SELECT id INTO v_existing_profile_id
  FROM public.profiles
  WHERE work_email = v_email AND user_id IS NULL;

  IF v_existing_profile_id IS NOT NULL THEN
    -- FIX: Preserve the existing profile's bu_id instead of overwriting with domain lookup.
    -- The profile was created by an admin with a specific BU — that is the source of truth.
    -- Only set user_id, onboarding, and user_type.
    UPDATE public.profiles
    SET user_id = NEW.id,
        onboarding_completed = false,
        -- bu_id is intentionally NOT overwritten here
    ...
    -- Use the profile's existing bu_id for membership creation below
    SELECT bu_id INTO v_bu_id FROM public.profiles WHERE id = v_existing_profile_id;
  ELSE
    INSERT INTO public.profiles (
      ...
      'RS',
      CURRENT_DATE,
      CASE WHEN v_is_external THEN 'external' ELSE 'active' END,
  ...
  -- Create BU membership if not exists
  INSERT INTO public.bu_user_memberships (user_id, profile_id, bu_id, role_in_bu, is_default)
  VALUES (NEW.id, v_new_profile_id, v_bu_id, 'member', true)
  ON CONFLICT (profile_id, bu_id) WHERE deleted_at IS NULL
  DO UPDATE SET user_id = NEW.id, is_default = true;

  -- Link external user to partner contact
  ...
  PERFORM set_config('app.internal_call', '', true);

  RETURN NEW;
END;
$$;

-- Part 2: Remove jetimob.com from Jet Experience's allowed_email_domains
UPDATE bu_units 
SET allowed_email_domains = array_remove(allowed_email_domains, 'jetimob.com')
