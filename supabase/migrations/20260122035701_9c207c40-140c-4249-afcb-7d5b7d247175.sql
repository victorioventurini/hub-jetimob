...
-- Fix can_view_ticket: For private tickets, check ALL participation paths before returning false
-- The issue was that CHECK 1 returned false for private tickets before checking partner_contact participation
...
BEGIN
  ...
  IF v_is_bu_member THEN
    CASE v_ticket.visibility
      ...
      WHEN 'private' THEN RETURN false;  -- Already checked all participation paths above
      ELSE RETURN false;
    END CASE;
  END IF;
...
COMMENT ON FUNCTION public.can_view_ticket IS 'Checks if a user can view a ticket. Checks: 1) Creator/owner, 2) Internal participant, 3) External participant via auth.uid, 4) Profile with partner_contact participation, 5) Visibility rules for non-participants.';
