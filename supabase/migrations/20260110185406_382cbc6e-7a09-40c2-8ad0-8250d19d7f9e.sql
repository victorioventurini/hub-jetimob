-- Fix search_mention_candidates function to use correct column names from profiles table
  ...
  WITH internal_users AS (
    ...
    WHERE 
      ...
      AND (v_search = '' OR 
       ...
       unaccent(lower(coalesce(p.first_name, ''))) LIKE '%' || v_search || '%' OR
       unaccent(lower(coalesce(p.last_name, ''))) LIKE '%' || v_search || '%' OR
