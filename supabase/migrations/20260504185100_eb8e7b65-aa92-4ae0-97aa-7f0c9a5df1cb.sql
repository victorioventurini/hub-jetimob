...
BEGIN
  ...
  -- Detect external user via partner_contacts + active BU association
  ...
  -- Pre-existing profile imported by admin
  ...
  VALUES (NEW.id, v_new_profile_id, v_bu_id, 'member'::public.app_role, true)
  ON CONFLICT (profile_id, bu_id) WHERE deleted_at IS NULL
  DO UPDATE SET user_id = NEW.id, is_default = true;

  -- Link external user to partner_contacts via user_id (no profile_id column exists)
