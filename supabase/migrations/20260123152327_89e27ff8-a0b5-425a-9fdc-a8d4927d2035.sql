-- Fix rpc_home_dashboard_data: usar valores válidos do enum okr_rag_status
-- Enum okr_rag_status tem: green, yellow, red, not_started
-- 'completed' NÃO existe no enum e causa erro 400
...
BEGIN
  ...
  -- Enum okr_rag_status: green, yellow, red, not_started
  SELECT 
  ...
  -- FIXED: Usar status != 'green' ao invés de 'completed' (que não existe no enum)
  -- KRs que ainda precisam de check-in são aqueles não-green e não cancelados
  SELECT 
  ...
  WHERE bu_id = p_bu_id
    ...
    AND status != 'green'  -- FIX: era 'completed', agora usa 'green' (objetivo atingido)
    AND (v_user_team_id IS NULL OR team_id = v_user_team_id);
