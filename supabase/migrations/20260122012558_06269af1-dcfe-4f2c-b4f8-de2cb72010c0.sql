-- ============================================================
-- FIX: can_view_ticket must support external users (partner_contacts)
-- ============================================================
-- External users don't have profiles.id - they use partner_contacts.user_id (auth.users.id)
-- The function now checks ticket_participants.partner_contact_id for external access
-- ============================================================

CREATE OR REPLACE FUNCTION public.can_view_ticket(p_ticket_id uuid, p_profile_id uuid)
RETURNS boolean
LANGUAGE plpgsql
...
BEGIN
  -- Get ticket info
  SELECT bu_id, visibility, visibility_team_ids, visibility_squad_ids, visibility_user_ids,
  ...
  -- =====================================================
  -- CHECK 2: External user (partner_contact)
  -- =====================================================
  -- Get auth.uid() to check if it's an external user
  v_auth_uid := auth.uid();
  ...
  IF v_auth_uid IS NOT NULL THEN
    ...
    -- Check if external user is a participant
...
COMMENT ON FUNCTION public.can_view_ticket(uuid, uuid) IS 
  'Checks if a user (internal via profile_id OR external via partner_contact) can view a ticket. External users need to be active participants.';