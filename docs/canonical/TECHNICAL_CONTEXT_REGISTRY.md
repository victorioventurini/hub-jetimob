...
**Versão:** 3.28.0  
**Última atualização:** 2026-04-21 (v3.28.0 - Onda 4 Ritos Semanais ATIVA: `pre-weekly` e `weekly` registrados como `WizardPersona` em `v2`; SSOT estrutural completo (stepDefinitions/labels/completion/visibility); novo componente unificado `PreparationStatusCard` (5 modos, 4 estados, transparência pública dos pendentes); agente `curador-orquestrador` ativo em `ai_agents` (genérico por função cognitiva, conforme `AI_AGENTS_PHILOSOPHY.md`). Containers ricos dos novos ritos + aplicação retroativa do card nos 6 ritos existentes pendentes em entrega dedicada. Sem mudança em janelas, permissões ou integrações existentes.)
