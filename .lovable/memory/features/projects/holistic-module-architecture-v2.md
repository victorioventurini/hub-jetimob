O módulo de Projetos (v1.6) gerencia iniciativas estratégicas com marcos (milestones), cronograma e indicadores de saúde (late/overdue, at_risk/<7 dias, on_track), diferenciando-se das 'okr_initiatives'. **Milestones têm responsável obrigatório** (`project_milestones.owner_id NOT NULL` desde 2026-04-24; backfill usou `project.owner_id` como fallback). UI bloqueia criação/edição sem responsável (`BuUserSelect allowNone={false}`); hooks fazem guard defensivo.
